...
Если вы инженер в крупной компании (особенно если ваша организация поставляет свои услуги в виде SaaS решений), то вам, так или иначе, надо будет решать задачу мониторинга работы всех ваших PostgreSQL. Очень часто бывает так что на Postgre SQL завязан важный с т.з. финансовых рисков функционал организации, поэтому крайне желательно не только иметь мониторинг, но и получать уведомления когда что-то идет не так (или что-то пойдет не так в ближайшем будущем).
...
По сути это агент, который будет собирать метрики PostgreSQL и отдавать их в Prometheus с локального endpoint (Prometheus опрашивает этот адрес каждые n секунд и складывает в свою time series БД).
...
##### Сбор метрик:
postgres_exporter выполняет SQL-запросы к системным представлениям (например pg_stat_database, pg_stat_activity, pg_stat_replication, pg_stat_bgwriter, и т.д.).
...
> Похожим образом работают все другие виды экспортеров для prometheus. У него есть масса различных экспортеров для разных систем, в частности mysql_exporter, mongodb_exporter, redis_exporter и пр.
...
2. Открываем раздел Configuration → Data sources → Add data source → Prometheus
...
Выбираем понравившийся и в Grafana выбираем: Dashboards → Import → вставь ID (9628 или 455) → выбери источник Prometheus.
...
Далее задать порог, длительность сколько держится условие, список получателей по email, Slack и т.п.

Так-же алертинг можно настраивать в Prometheus. Визуально там сделать не получится. Придется прописывать все алерты в yaml файлике и подключать ссылку на него в prometheus.yml.
В общем чтобы настроить все по красоте понадобится потратить много времени и сил.

## Готовые opensource решения
Кто не готов разбираться и настраивать все самостоятельно, получив на выходе достаточно скудный функционал, могут попробовать opensource решения разработанные именно для мониторинга PostgreSQL. Далее рассмотрим два самых популярных из них.

### Percona Monitoring and Management (PMM)

https://www.percona.com/software/database-tools/percona-monitoring-and-management 

Довольно продвинутое решение для мониторинга PostgreSQL (+ еще MySQL и MongoDB) с базовыми возможностями аналитики запросов, готовыми дашбордами, рекомендациями и аннотациями. Подходит для production-сред с потребностью в централизованном мониторинге и диагностике.
...
#### Архитектура: клиент‑серверная модель
 ...
 - PMM Server (центральный сервер мониторинга):
   ...
   - Визуализация и дашборды через Grafana с разработанными Percona шаблонами для нее (да, они как веб морду использует Garafana);
   - Включает базовые возможности аналитики тяжелых запросов (т.н. Query Analytics) и Percona Advisors (рекомендации по производительности и безопасности);
...
- Включает Query Analytics и Advisors (чего нет в типовой связке Grafana + Prometheus);
...
Python (Flask) + psycopg. Работает в двух режимах - Desktop приложуха и Server (веб-приложение). Данные для виджетов берёт на лету из стандартных pg_stat_* представлений и системных функций PostgreSQL. Метрики не накапливает - только опрашивает и показывает on demand.
...
|||
| --- | --- |
| Дашборды | «Живые» графики: CPU/IO wait (условная оценка по состоянию PostgreSQL, а не реальные хостовые т.к. агентов нет), активные/idle сессии, число блокировок, tuples in/out, чекпоинты, WAL-активность. Данные берутся из `pg_stat_activity`, `pg_stat_database`, `pg_stat_bgwriter`, `pg_stat_replication` и др. |
...
- Простая установка: пакеты для Win/macOS/Linux, Docker-образ, Helm; работает и как Desktop, и как Web.
##### Минусы
...
В целом подходит для малых/средних инсталляций, где нет требований к ретроспективному мониторингу и алертингу, ну или использовать как дополнение к PMM/Prometheus-стэку если вы готовы городить зоопарк.
...
- Log Insights (ошибки, deadlocks, медленные запросы). Можно настраивать алерты в какой-нибуль Slack.
...
| Профилирование производительности      | История выполнения запросов, EXPLAIN-планы, выявление «тяжёлых» и часто вызываемых SQL.        |
...
Платформа для мониторинга и аналитики PostgreSQL корпоративного уровня. Цель которой - не просто отображать метрики, а комплексно управлять всем парком PostgreSQL, собирать и хранить метрики БД и хостов, анализировать запросы и конфигурацию, автоматизировать обслуживание, выдавать рекомендации и централизовать оповещения. Поддерживает мульти-тенантность (тенанты/воркспейсы), роли и гибкое разграничение доступа.
...
- публикуют изменения метрик в шину сообщений NATS.
...
- Кастом-дашбордов нет: ориентирована на встроенные панели; гибкость ниже, чем у чистого Grafana-стека (зато меньше ручной настройки), но в планах есть и реализация кастомных дашбордов.
